...
To install Metatext in Eclipse, add the @http://lobid.org/download/tools/p2@ site under _Help_ -> _Install New Software_ ->  _Add..._ and install the 'Metatext' feature. After a restart, *.flow files will open with the Metatext editor. To execute a *.flow file, choose _Run_ -> _Run As_ -> _Metatext Workflow_.
...
bc. 
default files = ""; // set absolute path to the folder containing these files
