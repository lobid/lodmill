...
sh index.sh $NWBIB_SPATIAL $ES_SERVER $ES_CLUSTER $ALIAS
...
sh index.sh $ORGANISATIONS $ES_SERVER $ES_CLUSTER $ALIAS
...
sh index.sh $LOBID_TEAM $ES_SERVER $ES_CLUSTER $ALIAS
...
sh index.sh $LOBID_COLLECTIONS $ES_SERVER $ES_CLUSTER $ALIAS

GND=output/json-ld-gnd
sh convert.sh extlod/gnd/ $GND http://d-nb.info/gnd gnd-$TIME json-ld-gnd
sh index.sh $GND $ES_SERVER $ES_CLUSTER $ALIAS
...
sh index.sh $ITEMS $ES_SERVER $ES_CLUSTER $ALIAS
